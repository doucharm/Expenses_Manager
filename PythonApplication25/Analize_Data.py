...
def Analize_Expenses(main_menu,options_list,engine):
    "Analize expenses base on pre-set limit of each catagory and draw pie chart"
    data=All_Expenses(engine)
    catag=[0,0,0,0,0,0]
    over_spend=[0,0,0,0,0,0,0]
    for x in data:
        if int(x[3])>1000:
            over_spend[0]+=1 #overspend limit at 1000
    ...
    s_catag=list()
    s_option=list()
    for i in range(0,len(catag)): # remove catagories without any entries for diaplay
        if catag[i]!=0:
    ...
    over_spend[1]=round(sum(catag)/150,2) # rouding up data sources for display
    ...
    #labels for informations on spending limit
    o2.pack()
    o_all.pack()
    o3.pack()
    o4.pack()
